...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [roles, setRoles] = useState<RoleItem[]>(() => loadLocal('roles', INITIAL_ROLES));
  ...
  const [rolePermissions, setRolePermissions] = useState<Record<string, string[]>>(() => {
    ...
    (['admin_assistant', 'inventory_manager', 'cashier'] as const).forEach(r => {
      if (!updated[r]) {
  ...
  const login = (emailOrId: string | number, password?: string): boolean => {
    ...
    if (typeof emailOrId === 'number') {
      ...
      if (!matchedUser) {
        matchedUser = users.find(u => u.name.toLowerCase().includes(trimmed));
      }
    }

    if (matchedUser) {
      // Validate password if user has a custom password set, or allow default demo password
      if (password && matchedUser.password_hash && password !== matchedUser.password_hash && password !== 'password') {
        return false;
      }
  ...
  const sendPasswordResetRequest = (phone: string, staffName?: string) => {
    const adminUsers = users.filter(u => u.role_name === 'admin');
  ...
  const updateRole = (id: number, name: string) => {
    ...
    if (existing.name === 'admin') {
      showFlash('Cannot rename the core "admin" role.', 'error');
      return;
    }
  ...
  const deleteRole = (id: number) => {
    ...
    if (target.name === 'admin') {
      showFlash('Cannot delete the root "admin" role.', 'error');
      return;
    }
